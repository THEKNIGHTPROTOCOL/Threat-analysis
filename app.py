...
# - New Visualizations: Added a 3D PCA plot and a Cluster Profile Radar Chart.
# =========================================================================
...
with tab3:
    ...
    if len(numeric_cols) >= 2:
        ...
        st.markdown("An interactive 2D and 3D PCA plot helps visualize the clusters in a reduced dimension.")
        
        # PCA for 2D and 3D visualization of clusters
        pca_2d = PCA(n_components=2)
        X_pca_2d = pca_2d.fit_transform(X_scaled)
        pca_df_2d = pd.DataFrame(X_pca_2d, columns=['PCA Component 1', 'PCA Component 2'], index=X.index)
        pca_df_2d['Cluster'] = df['Cluster'].astype(str) # Convert to string for correct coloring

        # Create a scatter plot of the clusters in the 2D PCA space
        fig_2d = px.scatter(pca_df_2d, x='PCA Component 1', y='PCA Component 2', color='Cluster', 
                         hover_data=numeric_cols, # Show original data on hover
                         color_discrete_sequence=px.colors.qualitative.Plotly)
        fig_2d.update_layout(title_text=f"K-Means Clusters (K={k}, 2D PCA)", title_x=0.5)
        ...
        pca_df_3d = pd.DataFrame(X_pca_3d, columns=['PCA1', 'PCA2', 'PCA3'], index=X.index)
        pca_df_3d['Cluster'] = df['Cluster'].astype(str)
        
        fig_3d = px.scatter_3d(pca_df_3d, x='PCA1', y='PCA2', z='PCA3',
                               color='Cluster',
                               hover_data=numeric_cols,
                               color_discrete_sequence=px.colors.qualitative.Plotly)
        fig_3d.update_layout(title_text=f"K-Means Clusters (K={k}, 3D PCA)", title_x=0.5)
        st.plotly_chart(fig_3d, use_container_width=True)


        st.markdown("### 4. Cluster Profile Analysis")
        st.markdown("These charts visualize the **average values** for each feature across the different clusters. This is key to understanding the characteristics of each user segment.")
        ...
        with col1:
            ...
            for cluster in cluster_profiles.columns[1:]:
                fig_bar.add_trace(go.Bar(x=cluster_profiles['Feature'], y=cluster_profiles[cluster], name=f'Cluster {cluster}'))
        ...
        with col2:
            ...
            for cluster in cluster_profiles.columns[1:]:
                fig_radar.add_trace(go.Scatterpolar(
                    r=cluster_profiles[cluster],
                    theta=cluster_profiles['Feature'],
                    fill='toself',
                    name=f'Cluster {cluster}'
                ))
            fig_radar.update_layout(
                polar=dict(
                    radialaxis=dict(visible=True, range=[0, cluster_profiles.iloc[:, 1:].max().max() * 1.1])
                ),
                showlegend=True,
                title_x=0.5
            )
